...
{
	return scancodeNames[static_cast<std::size_t>(scancode)];
};

Scancode Keyboard::getScancodeFromName(const char* name)
{
	auto it = scancodeMap.find(std::string(name));
...
std::map<std::string, Scancode> Keyboard::buildScancodeMap()
{
	std::map<std::string, Scancode> scancodeMap;
	for (std::size_t i = 0; i <= static_cast<std::size_t>(Scancode::AUDIO_FAST_FORWARD); ++i)
	{
		if (scancodeNames[i] != nullptr)
		{
			std::string scancodeName = scancodeNames[i];
			scancodeMap[scancodeName] = static_cast<Scancode>(i);
		}
	}
...
const char* Keyboard::scancodeNames[] =
{
	nullptr, // UNKNOWN
	"A", // A
	"B", // B
...
std::map<std::string, Scancode> Keyboard::scancodeMap = Keyboard::buildScancodeMap();
