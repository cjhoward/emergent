...
#ifndef EMERGENT_MATH_VECTOR_MATH_HPP
#define EMERGENT_MATH_VECTOR_MATH_HPP
 ...
 */
template <class T, std::size_t N>
vector<T, N> squaredDistance(const vector<T, N>& p0, const vector<T, N>& p1);
 ...
 */
template <class T, std::size_t N>
T lengthSquared(const vector<T, N>& x);
...
template <class T, std::size_t N>
inline vector<T, N> distanceSquared(const vector<T, N>& p0, const vector<T, N>& p1)
{
	static_assert(std::is_floating_point<T>::value);
	return lengthSquared(sub(p0, p1));
}
...
template <class T, std::size_t N>
inline T lengthSquared(const vector<T, N>& x)
{
	static_assert(std::is_floating_point<T>::value);
	return dot(x, x);
}
...
#endif // EMERGENT_MATH_VECTOR_MATH_HPP
